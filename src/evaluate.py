...
from src.legal_moves import legal_token_ids
...
class ChessEvaluator:
    ...
    # Convert board to input format (dataset tokens string)
        moves_str = self._convert_board_to_moves(board)

    # Add BOS
        input_text = self.tokenizer.bos_token if not moves_str else (self.tokenizer.bos_token + " " + moves_str)
    ...
    # Pre-compute legal token ids ONCE for this position
        allowed_ids = legal_token_ids(board, self.tokenizer)
        if not allowed_ids:
            return None, self.max_retries  # no legal moves in vocab => fail safely
        ...
        # IMPORTANT: for a single token, prefer convert_ids_to_tokens over decode()
            move_token = self.tokenizer.convert_ids_to_tokens(next_id)

        # ---- Convert dataset token -> UCI robustly ----
            tok = move_token.strip()

        # Strip suffixes like (x), (+), (+*), (o)/(O)
            for suf in ["(x+*)", "(+*)", "(x+)", "(x)", "(+)", "(o)", "(O)"]:
                tok = tok.replace(suf, "")

        # Promotion in dataset is like "...=Q"
            promo = ""
            if "=" in tok:
                eq = tok.index("=")
                if eq + 1 < len(tok):
                    promo = tok[eq + 1].lower()
                tok = tok[:eq]

        # Now expected: WPe2e4 / BNg8f6 (at least 6 chars)
            if len(tok) < 6:
                continue

            uci_move = tok[2:4] + tok[4:6] + promo

        # Validate legality (should be legal already, but keep safety)
            try:
